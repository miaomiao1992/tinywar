...
use crate::core::mechanics::spawn::{HealthCmp, HealthWrapperCmp};
...
pub fn update_units(
    ...
    let objects: HashMap<Entity, (Vec3, f32, f32)> = unit_q
        .iter()
        .map(|(e, t, _, _, u)| (e, (t.translation, u.health, u.name.health())))
        .chain(building_q.iter().map(|(e, t, b)| (e, (t.translation, b.health, b.name.health()))))
        .collect();
    ...
    for (unit_e, unit_t, mut unit_s, heal, mut unit) in &mut unit_q {
        ...
        unit.action = match unit.action {
            ...
            Action::Heal(e) => objects
                .get(&e)
                .filter(|(pos, health, max_health)| {
                    unit_t.translation.distance(*pos) <= unit.name.range() * RADIUS
                        && health < max_health
                })
                .map(|(pos, _, _)| {
    ...
    // Update the health bar of the buildings and add fire animations
    for (building_e, _, building) in building_q.iter() {
                ...
                if building.health < building.name.health() {
                    *wrapper_v = Visibility::Inherited;
                    ...
                    for child in children_q.iter_descendants(wrapper_e) {
                        if let Ok((mut health_t, mut health_s)) = health_q.get_mut(child) {
                            if let Some(size) = health_s.custom_size.as_mut() {
                                size.x = 2. * INNER_HEALTH_SIZE.x * building.health
                                    / building.name.health();
                                health_t.translation.x = (size.x - 2. * INNER_HEALTH_SIZE.x) * 0.5;
                            }
                        }
                    }
                } else {
